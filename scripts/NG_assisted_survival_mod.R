...
# This model will be based on individual seedlings with errors distributed according to a bernoulli distribution.
...
add_ispline = FALSE
ispl_x_trt = TRUE 
ispl_k = 3 # how many knots? If knots are specified, their location is based on a kmeans clustering
ispl_int = FALSE # remove the intercept from the ispline part

mort_data$days <- pmax(mort_data$days, 0.5)
...
# specify distribution error model
distribution(mort_data$dead) <- bernoulli(mort_probs)

if(!is.null(mort_coefs$ispl_coef)) {
  
  env <- mort_coefs$env_coef
  
  ispl <- mort_coefs$ispl_coef
  
  mort_mod <- model(env, ispl, mort_coefs$site_sd, precision = "double") # can monitor whatever in here, e.g.,  site_sd 
  
  # hack the initial values so that the intercept is small
  #inits <- c(-5, rep(0, length(mort_mod$dag$example_parameters()) - 1))
  ...
  mort_inits <- replicate(4, 
                            initials(
                              ispl = rnorm(length(ispl), 
                                           ispl_init_means, 0.4),
                              # ispl = rep(seq(1.1, 1,  length.out = 5), 3), 
  ...
  mort_mod <- model(env, ply, site_sd, precision = "double") # can monitor whatever in here, e.g.,  site_sd 
...
#coda::varnames(mort_draws)[-c(1:length(ispl))] <- mort_design$coef_names

coda::varnames(mort_draws) <- 
  c("site_sd",
    rep("ply", 6),
    "intercept",
    "adj_mallee",
...
pdf(file = "gfx/fig3.pdf", width = 6.14, height = 5.87) #for Ecosphere
ggplot(data = mort_rab_med, 
       mapping = aes(x = days, y = 1 - mort_prob_pred_mean,
                     colour=treatment)) + 
  geom_line(aes(group = treatment), size=.8) + 
  ...
  theme_minimal() + 
  labs(x = "Days elapsed since planting", 
       y = "Probability of seedling survival", 
       colour = "Treatment") + 
  ...
  #   strip.background =element_blank(),
     strip.text = element_text(size = 11, margin = margin(5,5,10,5)),
...
browse_design <- prep_design(glm_formula = browse_lp, data_obj = browse_data,
                            add_ispl = add_ispl,  
...
if(length(browse_design$log_offset) == 6375) {
  ...
  distribution(browse_data_sum$browsed) <- binomial(browse_data_sum$total, browse_probs)
...
#plan(multisession)
browse_draws <- mcmc(browse_mod, warmup = 2000,
                     n_samples = 10000, 
...
pdf(filename = "gfx/fig5.pdf", width = 6.14, height = 7.87) #for Ecosphere
