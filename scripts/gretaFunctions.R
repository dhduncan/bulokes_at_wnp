...
prep_design <- function(glm_formula, data_obj, 
  ...
  if(!add_ispl) {
    ...
    design_matrix <- env_matrix
    
    ispl_bases <- NULL # this flag informs subsequent function
    
  } else {
    # construct monotonic spline element
    x <- data_obj$days # variable on which spline is based
    ...
    ispl_days <- iSpline(x, knots = knot_pos, df = df, degree = 3, 
                         ...
                         Boundary.knots = c(0, 410)) # just beyond extent of x, specified here so that it does contract when applied to prediction dataset of 365 days.
  ...
  list(matrix = design_matrix, site = site , log_offset = log_offset, 
       env_preds = env_preds, coef_names = coef_names,
...
#  coef[] <- normal(0, 2) # o lo que sea
  ...
  if(is.null(design_list$ispl_bases)) {
    ...
    ispl_coef <- normal(0, 1, truncation = c(0, Inf), 
  ...
  coef_names <- unique(colnames(design_list$matrix)[-grep(pattern = "ispl", colnames(design_list$matrix))])
  ...
  list(env_coef = env_coef, ispl_coef = ispl_coef, 
